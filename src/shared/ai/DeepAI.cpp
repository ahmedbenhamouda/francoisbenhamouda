...
namespace ai {
	DeepAI::DeepAI () {
		...
		position_cmd = state::Position(-1,-1);
		cmd_id = -1;
	}
	...
	void DeepAI::scoreDeplacement(engine::Command* command) {
		...
		if (jeu->selectedUnit->has_flag) {
			...
			for (state::Flag* flag : liste_flags_ennemie){
				if  (not(flag->is_owned)) {
					if (objectif.getX() == -1 or (flag->position - jeu->selectedUnit->position < shortest_distance)) { // aucun objectif predefini ou objectif precedent est plus loin
			...
			for (state::Unite* unit : jeu->etatJeu->getUniteList()){
				if  (unit->has_flag) {
					if (objectif.getX() == -1 or (unit->position - jeu->selectedUnit->position < shortest_distance)) {
	...
	void DeepAI::runMinMax() {
		...
		if (max_score == -1000) { // Use the minmax function for the first time
			engine->Clear();
			position_cmd = state::Position(-1,-1);
			cmd_id = -1;
			max_score = 0;
	...
	void DeepAI::run () {
		...
		if (jeu->joueurs[jeu->tour%nb_joueurs]->color == color) { //It's time to play !
			...
			if (jeu->simulation < 0) { // No one is simulating right now			
				if (jeu->selectedUnit) {
					if (max_score>=0 and (size_t)command_iter >= liste_commands.size()) { // Last level of the minmax tree
	...
	void DeepAI::ListeScore(){
		...
		if (engine->commands.size()>0) { // Vérifier qu'une commande n'a pas été passée car pas intéressante
			if (jeu->joueurs[jeu->tour%nb_joueurs]->score > max_score) { 
				...
				position_cmd = engine->commands[0]->getPos();
				cmd_id = engine->commands[0]->getId();
			}
		}
	...
	void DeepAI::selectFinalCommand(){
		...
		//std::cout<<"Position : x="<<pos.getX()<<", y="<<pos.getY()<<std::endl;
		if (cmd_id == 3) { // deplacement
			engine->addCommand(new engine::MoveUnitCommand(position_cmd));
		} else if (cmd_id == 4) { // attaque
			engine->addCommand(new engine::AttackUnitCommand(position_cmd));
		}
